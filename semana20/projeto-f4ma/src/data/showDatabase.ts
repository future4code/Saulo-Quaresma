...
export class ShowDatabase extends BaseDatabase implements ShowGateway {
  ...
  public async getShow(weekDay: ShowWeekDay, startTime: number, endTime: number): Promise<Show | undefined> {
    const result = await this.connection.raw(`
      SELECT * FROM ${this.showTableName}
      WHERE week_day = '${weekDay}' AND start_time = '${startTime}' AND end_time = '${endTime}'
    ;`)
